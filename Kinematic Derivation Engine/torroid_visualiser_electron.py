...
# Electron-like: Wide-open, expansive, light torque
    engine = TIRVisualizerEngine(
        center=[0.0, 0.0, 0.0],
        major_radius=1.6,       # Wide, relaxed macro boundary
        minor_radius=0.03,      # Razor-thin aerodynamic core profile
        vorticity_vec=[0.0, 0.0, 15.0], # Low vorticity / light geometric mass
        c=1.0
    )
    # Render with a wider frame to capture the sweeping electron disk
    engine.render_slice(x_bounds=(-3.0, 3.0), z_bounds=(-3.0, 3.0), res=150)