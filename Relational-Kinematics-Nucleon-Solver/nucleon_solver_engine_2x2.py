...
class TIR_NucleusEngine:
    ...
    def calculate_impedance_cost(self, flat_state):
        ...
        for i in range(self.total_entities):
            ...
            for j in range(i + 1, self.total_entities):
                ...
                # Gentle attraction at long-range; brutal exponential repulsion if cores overlap
                if distance < optimal_r:
                    ...
                    # Power-law fluid suction pulls entities toward their optimal touch perimeters
    ...
    def run_simulation(self):
        ...
        result = basinhopping(
            ...
            initial_flat_state,
            niter=50,             # Number of global coordinate jolts
            T=1.0,                # Simulated coordinate noise amplitude
        ...
        for i in range(self.total_entities):
            print(f"Entity [{i}] ({self.entity_types[i]}):")
            print(f"  -> Position Coordinate (X,Y,Z): {np.round(final_positions[i], 3)}")
            print(f"  -> Vector Spin Axis   (U,V,W): {np.round(final_spins[i], 3)}")
...
if __name__ == "__main__":
    ...
    solver = TIR_NucleusEngine(num_alphas=0, num_lone_neutrons=2, num_lone_protons=2)
