...
positions = np.array([
    [ 0.162,  1.794,  0.816],  # Entity 0 (alpha)
    [-1.390, -1.894,  1.585],  # Entity 1 (alpha)
    [-2.580,  1.880,  1.418],  # Entity 2 (alpha)
    [-1.252, -0.652, -0.019],  # Entity 3 (alpha)
    [ 0.445, -1.438, -0.810]   # Entity 4 (alpha)
])

spins = np.array([
    [ 0.058, -0.259,  0.964],  # Vector 0
    [ 0.273,  0.305,  0.912],  # Vector 1
    [-0.178,  0.797,  0.577],  # Vector 2
    [-0.187,  0.210,  0.960],  # Vector 3
    [-0.470,  0.081,  0.879]   # Vector 4
])
